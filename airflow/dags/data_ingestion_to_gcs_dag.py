...
dataset_file = "202010-citibike-tripdata.csv.zip"
dataset_url = f"https://s3.amazonaws.com/tripdata/{dataset_file}"
path_to_local_home = os.environ.get("AIRFLOW_HOME", "/opt/airflow/")
parquet_file = dataset_file.replace('.csv.zip', '.parquet')
BIGQUERY_DATASET = os.environ.get("BIGQUERY_DATASET", "citi_bikes")

def unzip_file(src_file, dest_file):
    zipfile = zipfile.ZipFile(src_file, "r")
    zipfile.extractall(dest_file)
...
with DAG(
    ...
    download_zip_file_task = BashOperator(
        ...
        #bash_command=f"curl -sSLf https://s3.amazonaws.com/tripdata/202010-citibike-tripdata.csv.zip > 202010-citibike-tripdata.csv.zip"
    )
    ...
    download_zip_file_task >> unzip_dataset_task >> format_to_parquet_task >> local_to_gcs_task